...
void first_pass() {
  ...
  if(fe && !ne){
    strcpy(name, "default");
    printf("No basename provided. Defaulted to  %s\n", name);
  }
}
...
struct vary_node ** second_pass() {
  ...
  for(i = 0; i < lastop; i ++){
    if (op[i].opcode == VARY) {
      double sframe = op[i].op.vary.start_frame;
      double eframe = op[i].op.vary.end_frame;
      double d = (op[i].op.vary.end_val-op[i].op.vary.start_val)/(eframe-sframe);
      for (frame = sframe; frame <= eframe; frame ++){
	struct vary_node * knob = (struct vary_node*)malloc(sizeof(struct vary_node));
	strcpy(knob->name, op[i].op.vary.p->name);
	knob->value = op[i].op.vary.start_val + (frame - sframe) * d;
